# Markdown Blog ve CMS

Markdown taslakları hazırla, güvenli önizle ve çakışan düzenlemeleri fark ederek yayımla.

![Uygulama ekranı](docs/screenshot.png)

**Durum:** Çalıştırılabilir yerel temel sürüm (v0.1). Yerel içerik yönetimi.
...
- Temizlenmiş HTML önizleme
- Yayımlama ve public JSON
- Version ile iyimser kilitleme
- Yazı sahipliği kontrolü

## Kapsam sınırı

Public uç nokta JSON döndürür; ayrı herkese açık blog sayfası ve SEO render yok. Görsel yükleme, etiket ve revizyon geçmişi sonraki sürüm. Slug değişmez; kısa UUID soneki kullanılır.
...
Markdown parser çıktısı sanitize-html ile temizlenir. Önizleme ve public endpoint aynı render fonksiyonunu çağırır. Güncelleme WHERE version=? koşuluyla yapılır ve version artırılır; sıfır satır değişirse 409 döner. Yazı yayımlanınca taslak değil published durumuna geçer. Kimlik bilinmesi yazı sahipliği kontrolünü atlatmaz.
...
| --- | --- | --- |
| GET /public/posts/:slug | Gövde yok | 200 |
| GET /api/posts | Gövde yok | 200 |
| POST /api/preview | JSON: markdown | 200 |
| POST /api/posts | JSON: title, markdown | 201 |
| PATCH /api/posts/:id | JSON: title, markdown, version | 200 |
| POST /api/posts/:id/publish | Gövde yok | 200 |
| DELETE /api/posts/:id | Gövde yok | 200 |
...
```text
POST /api/posts
{"title":"Vue notları","markdown":"# Merhaba"}
201 {"id":"<post-id>","slug":"vue-notlar-..."}
```

## Kabul senaryoları

- [ ] Taslak public endpoint üzerinden okunamamalı.
- [ ] Eski version ile yazma 409 dönmeli.
- [ ] Script ve javascript: bağlantısı önizlemede etkin olmamalı.
- [ ] Başka kullanıcı yazıyı silememeli.

## İlk gün yapacağın çalışma

Bir taslak oluştur ve iki sekmede aç. İlk sekmede kaydet; ikinci sekmede eski version ile kaydın neden reddedildiğini Network panelinde incele.

## Sonraki geliştirmeler

- [ ] Okuyucu için public sayfa ekle
- [ ] Etiket ve arama ekle
- [ ] Revizyon geçmişini ayrı tabloya taşı
- [ ] Sunucu render ve SEO meta verilerini ekle
...
Optimistic locking hangi durumda daha uygundur? Markdown parser kullanmak neden HTML temizliğinin yerine geçmez?
